...
**Ответ.** Класс всегда имеет как минимум один конструктор. Если он не определен явно, то создается автоматически  
...
**Ответ.** Используя ключевое слово ``this``, можно строить одни конструкторы на основе использования возможностей других, при этом ``this`` должен быть в первой строчке  
**Источник.** Блинов "Java from EPAM" 2020
 
Как и в каком месте можно вызвать конструктор суперкласса?  
**Ответ.** Ключевое слово ``super`` применяется для обращения к конструктору супер-класса и для доступа к полю или методу суперкласса из подкласса.  
**Источник.** Блинов "Java from EPAM" 2020

Как и в каком месте можно вызвать конструктор суперкласса?  
**Ответ.** Если ваш метод переопределяет один из методов своего суперкласса.
...
Обоснуйте ответ.  
**Ответ.** Нет, если используется ``this()`` или ``super()``, то обращение должно к нему быть первой строкой конструктора. Одновременно они не могут быть первой строкой конструктора.

Может ли быть приватный уровень доступа у конструктора? Если да, то для каких целей он необходим?  
**Ответ.** Может, использование-делегирование конструкторов, у вас может быть один конструктор, который принимает множество различных вариантов, что на самом деле является деталью реализации, затем ваши оставшиеся конструкторы делегируют ему.

Какие модификаторы может иметь конструктор?
**Ответ.** ``public``, ``private``, ``protected``, ``default``  
...
**Ответ.** Фрагмент 2. В Фрагменте 1 мы создаем метод ``Null()``, тип возвращаемого значения ``Null``, результат возвращения ``null``.
...
**Ответ.** Метод ```String makeRandomName()``` не может быть вызван до создания объекта.
Исправление данной ошибки можно добавив методу ``makeRandomName()``, ключевое слово ``static``.  
...
Объясните, почему?  
**Ответ.** Измениться, т.к когда мы создаем новый объект класса Int в методе inc(). Цикл жизни этого объекта находится в блоке этого метода.
...
**Ответ.** Если в методе объявлена локальная переменная или параметр метода с таким же именем, как и поле класса, то для обращения
...
Еесли в методе объявлена локальная переменная
или параметр метода с отличающимся именем от имени поля класса.  
...
**Ответ.** Статические методы используются при необходимости придать функциональности метода признак «окончательности», «неизменности» реализации алгоритма для данного класса. Вызов статического метода не может быть никаким образом заменен другой реализацией. Выбор статического метода раз и навсегда определяется на этапе компиляции.  
**Источник.** Блох 81-82стр.
 
Перечислите случаи, когда используется логический блок?
В каких случаях можно обойтись без него. Если можно, то каким образом?  
**Ответ.** Логические блоки могут применяться для проверки и инициализации параметров конкретного объекта и для сокращения количества кода, если одинаковый код присутствует в каждом конструкторе.  
**Источник.** Блох 85стр.

Что входит в сигнатуру метода?  
**Ответ.** Сигнатура метода — это имя метода плюс параметры (причем порядок параметров имеет значение).  
**Источник.** https://docs.oracle.com/
...
**Ответ.** Да, необходимо создать утилитный класс и в нем реализовать метод для преобразования финансовой величины
...
**Ответ.** По причине недоступности указателя ``this`` статические поля и методы не могут обращаться к нестатическим полям и методам напрямую, так как они не «знают», к какому объекту относятся, да и сам экземпляр класса может быть не создан. Для обращения к статическим полям и методам достаточно имени класса, в котором они определены.   
**Источник.** Java from EPAM : учеб.-метод. пособие / И. Н. Блинов, В. С. Романчик.

Почему нельзя объявить статическое поле внутри метода?  
**Ответ.** Для работы со статическими полями используются статические методы, объявленные со спецификатором ``static``. Поэтому нельзя объявлять статические поля непосредственно в методе.  
**Источник.** Java from EPAM : учеб.-метод. пособие / И. Н. Блинов, В. С. Романчик.

Перечислите, какой код в классе можно выполнить до создания объекта.  
**Ответ.** Статические методы, блок инициализации.  
**Источник.** Java from EPAM : учеб.-метод. пособие / И. Н. Блинов, В. С. Романчик.
...
Способ 1: сделать поля класса константами.  
...
**Ответ.**  В методе ``printMaterial()`` по ссылке ``material`` имеется доступ к объекту ``new Material("steel",7850)``, так как если бы у нас создавался новый объект, то код выглядел бы следующим образом:
```java
private static void printMaterial(final Material material) {
        System.out.println(material);
    }
    public static void main(String[] args) {
        printMaterial(new Material("steel", 7850));
    }
```
...
```
Реализация  методов при условии, что класс Complex является иммутабельным
```java
public Complex plus(double re, double im) {
    return new Complex(this.re + re, this.im + im);
}
...
**Ответ.** toString метод используется для получения строкового представления объекта. Каждый раз, когда мы пытаемся вывести ссылку на ```Objec`t, вызывается метод ``toString()``. Если мы не определили в классе метод ``toString()``, то будет вызван метод ``toString()`` класса ``Object``.  
...
В каких случаях он вызывается?  
**Ответ.** Метод ``equals()`` указывает, равен ли какой-либо другой объект объекту у которого вызывается этот метод. Метод ``equals()`` вызывается в случае надобности сравнить два объекта или же сравнить строки.  
...
**Ответ.** ``Equals()`` Метод класса`` Object`` реализует наиболее разборчивых возможное отношение эквивалентности на объектах; то есть для любых ненулевых ссылочных значений Х и У этот метод возвращает ``true`` тогда и только тогда, когда Х и У ссылается на один и тот же объект ( Х == У имеет значение ``true``).  
**Источник.** https://docs.oracle.com/javase/7/docs/api/java/lang/Object.html#equals(java.lang.Object)
...
• непротиворечивость — при многократном вызове метода для двух не подвергшихся изменению за это время объектов возвращаемое значение всегда
...
Обоснуйте ответ.
**Ответ.** Нет,не гарантирует,потому что задача garbage collector: 
1. найти мусор(неиспользуемые объекты) 
2. удалить мусор.Следовательно garbage collector помогает избавиться от лишней выделенной памяти,
а не гарантирует что приложение не будет иметь пролемы с ней.
...
**Ответ.** autoboxing задействован при присвоении значения примитивного типа переменной соответствующего класса-обёртки(``Integer a = 3, b =7;c = 7``);
           unboxing задействован в выражении, в котором оба аргумента являются экземплярами классов-обёрток(a + b);  
