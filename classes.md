...
**Ответ.** Членами класса являются его поля и методы, вложенные классы.  
...
**Ответ.** ``private`` - член класса доступен только из кода в том же классе;``protected`` - доступен из любого места в текущем классе или пакете или в производных классах, даже если они находятся в других пакетах;``public`` - Поля и методы видны другим классам из текущего пакета и из внешних пакетов; по умолчанию(``package-private``) - поля или методы видны всем классам в текущем пакете.   
...
**Ответ.** Вызов конструктора суперкласса должен быть первой строкой в конструкторе подкласса. Синтаксис вызова конструктора суперкласса: ``super()`` или же: ``super (parameter list)``. С ``super()`` вызывается конструктор суперкласса без аргументов. С ``super(parameter list)`` вызывается конструктор суперкласса с соответствующим списком параметров.  
...
Объясните, почему?  
**Ответ.** Измениться, т.к когда мы создаем новый объект класса Int в методе inc(). Цикл жизни этого объекта находится в блоке этого метода, т.е в методе `inc(Int param)` param переменная уже ссылается на новый объект. Меняется значение нового объекта.        
...
1.	Когда у переменной экземпляра класса и аргумента метода/конструктора одинаковые имена;
...
3.  Когда нужно сделать класс мутабельным.   
      Можно обойтись без this если задать имена параметров отличные от имен переменных класса.      
...
**Ответ.** Статические блоки могут применяться для проверки и инициализации базовых параметров, необходимых для функционирования приложения или класса.
Нестатичексие логические блоки могут применяться для проверки и инициализации параметров конкретного объекта и для сокращения количества кода, если одинаковый код присутсвует в каждом конструкторе.
При этом значения констант можно инициализировать и при описании класса, не прибегая к использованию статических блоков.При создании объекта можно заменить использование статического блока статическим методом.  
...
**Ответ.** Метод `Main()` станет методом экземпляра класса .
